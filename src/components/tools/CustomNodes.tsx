...
export function ResistorNode({ data, isConnectable, selected }: any) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]' : ''}`}>
      ...
      <div className="relative w-[60px] h-[30px] flex items-center justify-center text-amber-500">
        <Handle type="source" position={Position.Left} id="T1" style={{...HStyle, left: -5}} isConnectable={isConnectable} />
        <svg width="60" height="20" viewBox="0 0 60 20" className="drop-shadow-lg">
        ...
        <Handle type="source" position={Position.Right} id="T2" style={{...HStyle, right: -5}} isConnectable={isConnectable} />
      </div>
...
export function CapacitorNode({ data, isConnectable, selected }: any) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]' : ''}`}>
      ...
      <div className="relative w-[60px] h-[30px] flex items-center justify-center text-emerald-500">
        <Handle type="source" position={Position.Left} id="T1" style={{...HStyle, left: -5}} isConnectable={isConnectable} />
        <svg width="60" height="20" viewBox="0 0 60 20" className="drop-shadow-lg">
...
export function BatteryNode({ data, isConnectable, selected }: any) {
  return (
    <div className={`flex flex-col items-center ${selected ? 'drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]' : ''}`}>
      ...
      <div className="relative w-[60px] h-[40px] flex items-center justify-center text-blue-500">
        <Handle type="source" position={Position.Left} id="T1" style={{...HStyle, left: -5}} isConnectable={isConnectable} />
        <span className="absolute left-2 top-0 text-[10px] font-black text-blue-400">+</span>
        ...
        <span className="absolute right-2 top-1 text-[12px] font-black text-blue-400">-</span>
        <Handle type="source" position={Position.Right} id="T2" style={{...HStyle, right: -5}} isConnectable={isConnectable} />
      </div>
      <span className="text-[9px] text-neutral-400 mt-1">{data.label}</span>
